...
def generate_synthetic_data(rows: int = 1_000, seed: int = 2026) -> list[dict]:
    ...
    for _ in range(rows):
        ...
        prior_abandonment_rate = (prior_abandons + 1) / (prior_verifications + 6)
        ...
        verify_completed = bool(
            verify_fraud_loss > 0
            or (not fraud and rng.random() >= legitimate_abandonment)
        )
        allow_cost = allow_fraud_loss
        verify_cost = (
            verify_fraud_loss
            + settings.verify_cost
            + (amount * settings.abandonment_cost_rate if not fraud and rng.random() < legitimate_abandonment else 0.0)
        )
