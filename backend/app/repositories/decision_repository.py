...
class DecisionRepository:
    ...
    async def activity_for_account(self, account_id: str, now: datetime):
        """Windowed decisions of all outcomes, including pending and abandoned.

        Timestamps generated by this service are UTC ISO strings. This is a
        snapshot of persisted decisions, not an atomic request rate limiter.
        """
        from app.services.activity_service import activity_from_records
        from app.core.config import settings as config

        now = now.astimezone(timezone.utc)
        start = (now - timedelta(seconds=config.activity_window_seconds)).isoformat()
        try:
            collection = self._collection()
            if collection is not None:
                records = [self._clean(row) async for row in collection.find({
                    "account_id": account_id,
                    "$or": [
                        {"created_at": {"$gte": start, "$lte": now.isoformat()}},
                        {"abandoned_at": {"$gte": start, "$lte": now.isoformat()}},
                    ],
                })]
                return activity_from_records(records, now)
        except Exception as exc:
            _log_mongo_failure("account activity query", exc)
        return activity_from_records(
            [row for row in self._records.values() if row.get("account_id") == account_id], now,
        )
