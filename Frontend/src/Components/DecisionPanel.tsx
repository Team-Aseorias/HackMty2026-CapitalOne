...
export default function DecisionPanel({ decision: d, onUpdate, onNew, onViewEvaluation, onViewFeed }: { decision: DecisionRecord; onUpdate: (d: DecisionRecord) => void; onNew: () => void; onViewEvaluation: () => void; onViewFeed: () => void }) {
  ...
  return <main className="decision-page">
    ...
    <section className="explanation-card"><div><h2>Contexto de la recomendación</h2>
      ...
      {evidence && <p>Historial registrado: {evidence.resolved_verifications} verificaciones con resultado, {evidence.reported_abandons} abandonos reportados. Tasa histórica suavizada: {percent(evidence.smoothed_abandonment_rate)}; mínimo de {evidence.minimum_history} resultados para incorporar la tasa personal. La tasa histórica no es la probabilidad predicha de abandonar la compra actual.</p>}
      {!!evidence?.capped_fields?.length && <p>Se aplicó el límite del historial admitido por el modelo. Entradas efectivas: {evidence.model_prior_verifications} verificaciones y tasa de abandono {percent(evidence.model_abandonment_rate ?? null)}. Se conserva el historial completo. Este ajuste limita la extrapolación y no implica validación para este perfil.</p>}
      ...
      <p>{d.demo_profile_key ? 'Contexto del perfil de demostración.' : d.context_source === 'nessie' ? 'Contexto de cuenta consultado en Nessie.' : 'Contexto local: no se confirmó contexto en Nessie.'} {d.persistence_source === 'mongo' ? 'Decisión guardada en MongoDB.' : 'Decisión en memoria; puede perderse al reiniciar.'}</p>
